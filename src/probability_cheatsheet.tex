...
% Comment out the following 6 lines for B&W
\titleformat{\section}
{\color{blue}\normalfont\large\bfseries}
...
% Command pandoc tightlist: https://tex.stackexchange.com/a/258486 
...
\begin{document}
    ...
    \begin{multicols*}{3}
        ...
        \begin{center}
        % comment out line with \color{blue} and uncomment next line for B&W
		{\color{blue} \Large{\textbf{Probability Cheatsheet v3.0}}} \\
        % {\Large{\textbf{Probability Cheatsheet v3.0}}} \\
        ...
        \subsection{Sampling Table}\label{subsec:sampling-table}
        \vspace{-3ex}
        \begin{minipage}{\linewidth}
            \centering
            \includegraphics[width=30ex]{../figures/jar.png}
        \end{minipage}
        %  \begin{center}
        ...
        \[P_{\textrm{naive}}(A) = \frac{\textnormal{number of outcomes favourable to $A$}}{\textnormal{number of outcomes}}\]
...
% 		\end{minipage}
        ...
        \begin{description}
            ...
            % Similarly, let's say we know the joint PDF of $U$ and $V$ but are also interested in the random vector $(X, Y)$ defined by $(X, Y) = g(U, V)$. Let
            % $$
            % J = \det \frac{\partial (u,v)}{\partial (x,y)}  =
            %     \det \begin{pmatrix}
            ...
            % \[f_{X,Y}(x, y) = f_{U,V}(u,v) \left|J\right| \]
        ...
        Let $G:\mathbb{R}^n \to \mathbb{R}^n$ be a continuous and invertible function with continuous partial derivatives and let $H=G^{-1}$. Let $\mathbf{Y}=G(\mathbf{X})$ and $\mathbf{X}=G^{-1}(\mathbf{Y})=H(\mathbf{Y}) = [H_1(\mathbf{Y}), \dots H_n(\mathbf{Y})]^T$.
        ...
        \section{Random Vectors}\smallskip \hrule height 2pt \smallskip
        Let $\mathbf{X} = [X_{1}, X_{2}, \ldots, X_{n}]^T$ and $\mathbf{Y} = [Y_{1}, Y_{2}, \ldots, Y_{n}]^T$ be random vectors. Then they have the following properties:
        \begin{description}
            \item[CDF]
            $F_{\mathbf{X}}(\mathbf{x}) =F_{X_{1}, \ldots, X_{n}}\left(x_{1}, \ldots, x_{n}\right) = P\left(X_{1} \leq x_{1}, \ldots, X_{n} \leq x_{n}\right)$
            \item[PDF]
            $f_{\mathbf{X}}(\mathbf{x})=f_{X_{1}, X_{2}, \ldots, X_{n}}\left(x_{1}, x_{2}, \ldots, x_{n}\right)$ (for jointly continuous $X_{i}$'s)
            \item[Expectation]
            $\mathbf{E}\mathbf{X} = [EX_{1}, EX_{2}, \ldots, EX_{n}]^T$
            \item[Correlation]
            $\mathbf{R}_{\mathbf{X}}=\mathbf{E}\left[\mathbf{X} \mathbf{X}^{\mathrm{T}}\right]$ where $[\mathbf{R}_{\mathbf{X}}]_{ij} = E[X_i X_j]$. Properties:
            \begin{itemize}
                \item $\mathbf{R}_{\mathbf{X}}$ is \textbf{positive semidefinite}.
                \item $\mathbf{R}_{\mathbf{X}}$ is \textbf{positive definite} if and only if all its eigenvalues are positive or equivalently $\operatorname{det}\left(\mathbf{C}_{\mathbf{X}}\right)>0$.
            \end{itemize}
            \item[Covariance]
            $\mathbf{C}_{\mathbf{X}}=\mathbf{E}\left[(\mathbf{X}-\mathbf{E} \mathbf{X})(\mathbf{X}-\mathbf{E} \mathbf{X})^{\mathrm{T}}\right]=\mathbf{R}_{\mathbf{X}}-\mathbf{E} \mathbf{X} \mathbf{E} \mathbf{X}^{\mathbf{T}}$ where $[\mathbf{C}_{\mathbf{X}}]_{ij} = Cov[X_i, X_j] = E[X_i X_j] - E[X_i] E[X_j]$.
            \item[Cross-covariance]
            $\mathbf{R}_{\mathbf{X Y}}=\mathbf{E}\left[\mathbf{X} \mathbf{Y}^{\mathbf{T}}\right]$
            \item[Cross-correlation]
            $\mathbf{C}_{\mathbf{X Y}}=\mathbf{E}\left[(\mathbf{X}-\mathbf{E X})(\mathbf{Y}-\mathbf{E Y})^{\mathbf{T}}\right]$
            \item[Method of Transformations] Let $\mathbf{Y}=G(\mathbf{X})$ where $G: \mathbb{R}^{n} \mapsto \mathbb{R}^{n}$ is a continuous and invertible function with continuous partial derivatives and let $H=G^{-1}$ so that $\mathbf{X}=G^{-1}(\mathbf{Y})=H(\mathbf{Y})$. Then, the PDF of $Y$ is given by
            \[
        ...
        \begin{minipage}{\linewidth}
            \centering
            \includegraphics[width=1.8in]{../figures/diff_types_of_convgnc_color.png}
        \end{minipage}

        Convergence of a sequence of random variables \(X1, X2, X3, \dots \) to
        random variable \(X\).
        ...
        \begin{description}
            \item[Notation] \(X_n \ \xrightarrow{L^{\large r}}\ X\)
            \item[Meaning] \(\lim_{n \rightarrow \infty} F_{X_n}(x)=F_X(x)\), for all \(x\) at which \(F_X(x)\) is continuous.
            \item[Examples] \emph{Central Limit Theorem}
        \end{description}
        ...
        \begin{description}
            \item[Notation] \(X_n \ \xrightarrow{L^{\large r}}\ X\) if,
            \item[Meaning] \(\lim_{n \rightarrow \infty} P\big(|X_n-X| \geq \epsilon \big)=0, \qquad \textrm{ for all }\epsilon>0\)
            \item[Examples] \emph{Weak Law of large Numbers}
        \end{description}

        \hypertarget{convergence-in-mean}{%
        \subsection{Convergence in mean:}\label{convergence-in-mean}}

        \begin{description}
            \item[Notation] \(X_n \ \xrightarrow{L^{\large r}}\ X\) if,
            \item[Meaning] \(\lim_{n \rightarrow \infty} E\left(|X_n-X|^{\large r}\right)=0\)
            \item[mean-square convergence] \(X_n \ \xrightarrow{m.s.}\ X\) is defined as \(X_n \ \xrightarrow{L^{\large 2}}\ X\)
        \end{description}
        ...
        \begin{description}
            \item[Notation] \(X_n \ \xrightarrow{a.s.}\ X\)
            \item[Meaning] \(P\left( \left\{s \in S: \lim_{n\rightarrow \infty} X_n(s)=X(s)\right\}\right)=1\), where \(S\) is the sample space of \(X_i\) and \(X\)
            \item[Criteria] Criteria for a.s. convergence:
            \begin{itemize}
                \tightlist
                \item \(X_n \ \xrightarrow{a.s.}\ X\) if \(\sum_{n=1}^{\infty} P\big(|X_n-X| > \epsilon \big) < \infty, \qquad \textrm{ for all }\epsilon>0\)
                \item \(X_n \ \xrightarrow{a.s.}\ X\) \emph{iff} \(\lim_{m\rightarrow \infty} P(A_m) =1, \qquad \textrm{ for all }\epsilon>0\) where,
                \(A_m=\{|X_n-X|< \epsilon, \qquad \textrm{for all }n \geq m \}.\)
            \end{itemize}
            \item[Examples] \emph{Strong Law of large Numbers}
        \end{description}

        \hypertarget{continuous-mapping-theorem}{%
        \subsection{Continuous Mapping
        Theorem}\label{continuous-mapping-theorem}}

        \begin{itemize}
            \tightlist
            \item For a \emph{continuous} \(h: \mathbb{R} \mapsto \mathbb{R}\)
            \item \(X_n \ \xrightarrow{d}\ X \implies h(X_n) \ \xrightarrow{d}\ h(X)\)
            \item \(X_n \ \xrightarrow{p}\ X \implies h(X_n) \ \xrightarrow{p}\ h(X)\)
            \item \(X_n \ \xrightarrow{a.s.}\ X \implies h(X_n) \ \xrightarrow{a.s.}\ h(X)\)
        \end{itemize}


        \section{Limit Theorems}\smallskip \hrule height 2pt \smallskip

        \subsection{Law of Large Numbers (LLN)}
        Let $X_1, X_2, \dots, X_n$ be i.i.d. r.v.s and let be $\bar{X}_n = \frac{X_1 + X_2 + \dots + X_n}{n}$ be the \textbf{sample mean}. Then the two variants of the \textbf{Law of Large Numbers} are:
        \begin{description}
            \item[Weak (WLLN)] $\bar{X}_n \xrightarrow{p} E[X_1]$
            \item[Strong (SLLN)] $\bar{X}_n \xrightarrow{a.s.} E[X_1]$
        \end{description}

        \subsection{Central Limit Theorem (CLT)}
        Let $X_1, X_2, \dots, X_n$ be i.i.d. with expected value $E[X_i] = \mu < \infty$ and variance $0 < Var[X_i] = \sigma^2 < \infty$ and let be $\bar{X}_n = \frac{X_1 + X_2 + \dots + X_n}{n}$ be the \textbf{sample mean}. Then, the random variable,
        \[ Z_{\large n}=\frac{\overline{X}-\mu}{ \sigma / \sqrt{n}}=\frac{X_1+X_2+...+X_{\large n}-n\mu}{\sqrt{n} \sigma} \]
        converges in distribution to the standard normal random variable as $n \to \infty$, i.e. $Z_n \xrightarrow{d} \N(0, 1)$ or,
        ...
        \end{minipage}
        % Let us say that $X$ is distributed $\Gam(n,\lambda)$. We know the following:
        \begin{description}
        ...
        % Let us say that $X$ is distributed $\chi^2(n)$. We know the following:
        \begin{description}
        ...
        % Let us say that $X$ is distributed $\chi^2(n)$. We know the following:
        \begin{description}
            \item[Story] If $Z \sim \N(0, 1)$ and $Y \sim \chi^2(n)$, where $n \in \mathbb{N}$ are independent r.v.s., then the r.v. $X = Z/\sqrt{Y/n}$ is said to have a t-distribution with $n$ \textit{degrees of freedom} shown by $X \sim T(n)$.
            \item[PDF] $f_X(x) = \frac{1}{2^{\frac{n}{2}} \Gamma\left(\frac{n}{2}\right)} x^{\frac{n}{2}-1} e^{-\frac{x}{2}}, \quad x \in [0, \infty)$
            \item[Moments] $E[X] = 0 \text{ if } n > 1 \quad Var[X] = \frac{n}{n-2} \text{ if } n > 2$
        \end{description}
        ...
        \begin{description}
            ...
            \item[PMF] $P(X=k) = {r + k - 1 \choose k}p^r(1-p)^k \quad k \in \{$0, 1, 2, \dots $\}$
            \item[Moments] $E[X] = \frac{r(1-p)}{p} \quad Var[X] = \frac{r(1-p)}{p^2}$
        \end{description}
        ...
        \begin{enumerate}
            ...
            \item $\chi^2(n) \sim \Gam\left(\frac{n}{2}, \frac{1}{2}\right)$
        ...
        \begin{enumerate}
            ...
            \item \textbf{Union-bound} $P\biggl(\bigcup_{i=1}^n A_i\biggr) \leq \sum_{i=1}^n P(A_i)$ for any events $A\_1, A\_2, ..., A_n$. Can be generalised using \textit{Inclusion-Exclusion Principle}
            \item \textbf{Markov} $P(X \geq a) \leq \frac{E|X|}{a}$ for $a>0$
            \item \textbf{Chebyshev} $P(|X - E[X]| \geq a) \leq \frac{Var[X]}{a^2}$ for $a>0$
            \item \textbf{Chernoff} Let $M_X(s) = E[e^{sX}]$ be MGF. Then, \\
        ...
        \[ \Gamma(\alpha) = \int_0^\infty t^{\alpha-1}e^{-t}\, dt \quad B(\alpha, \beta) = \frac{\Gamma(\alpha)\Gamma(\beta)}{\Gamma(\alpha + \beta)} = \int_0^1 t^{\alpha - 1}(1-t)^{\beta-1}\, dx \]
        Also, $\Gamma(\alpha+1) = \alpha \Gamma(\alpha)$, and $\Gamma(n) = (n - 1)!$ if $n$ is a positive integer.
...
\texttt{dchisq(x,n)}         & PDF $f(x)$ for $X \sim \chi^2(n)$           \\
...
\hline
\shortstack{Negative Binomial \\ \NBin($r, p$)} & \shortstack{$P(X=k) = {r + k - 1 \choose n}p^rq^k$ \\ $k \in \{$0, 1, 2, \dots $\}$} & $rq/p$ & $rq/p^2$ &  $(\frac{p}{1-qe^t})^r, \, qe^t < 1$\\
\hline
...
\hline
\shortstack{Poisson           \\ \Pois($\lambda$)} & \shortstack{$P(X=k) = \frac{\lambda^ke^{-\lambda}}{k!}$ \\ $k \in \{$0, 1, 2, \dots $\}$} & $\lambda$ & $\lambda$ & $e^{\lambda(e^t-1)}$ \\
\hline
\hline
...
\hline
\shortstack{Chi-Square        \\ $\chi^2(n)$} & \shortstack{$\frac{1}{2^{n/2}\Gamma(n/2)}x^{n/2 - 1}e^{-x/2}$\\$x \in (0, \infty) $} & $n$  & $2n$ & $(1 - 2t)^{-n/2}, \, t < 1/2$\\
\hline
\shortstack{Student-$t$       \\ $T(n)$} & \shortstack{$\frac{\Gamma((n+1)/2)}{\sqrt{n\pi} \Gamma(n/2)} (1+x^2/n)^{-(n+1)/2}$\\$x \in (-\infty, \infty)$} & $0$ if $n>1$ & $\frac{n}{n-2}$ if $n>2$ & doesn't exist\\
\hline
\end{tabular}
\end{center}
\end{document}
